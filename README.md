...
| 包 | 適合的專案 | 工作流 |
|----|-----------|--------|
| [dev/](flavors/dev/README.md) | 程式開發 | feature-dev、testing、refactor、investigation、dev-env、conventions、code-map |
...
| [multi-agent/](flavors/multi-agent/README.md) | 多 agent／session 協作 | inbox（信件）、resources（資源鎖）、dispatch（交接書）|
...
1. **合併**：上面這行。腳本複製 kernel、合入各包 `workflows/`、把片段貼進 AGENTS / WORKFLOWS / INDEX / common、非侵入式時改寫連結，最後印殘留清單（手動步驟見各包 README 與 [docs/non-invasive-import.md](docs/non-invasive-import.md)）。
...
| [CLAUDE.md](template/CLAUDE.md) | 轉址檔 → AGENTS.md（見「不只 Claude」）|
| [WORKFLOWS.md](template/WORKFLOWS.md) | 派發器：flavor 派發表貼入區＋kernel 內建表＋活狀態判別表 |
| [INDEX.md](template/INDEX.md) | 頂層結構地圖 |
| [STRUCTURE.md](template/STRUCTURE.md) | 被動結構參考：分層、膨脹即拆、四級成長、archive |
| [SESSION-LOG.md](template/SESSION-LOG.md) | 進度（open-only）|
...
| Cursor / Windsurf / Copilot / OpenCode / Amp / Aider… | 多數已直接讀 `AGENTS.md`；若不讀就 `--redirect <該工具的指示檔名>`（例 `.github/copilot-instructions.md`）| 依該工具，沒有就跑 `tools/` 腳本 |

## 升級既有專案

導入是複製後脫鉤。kernel 變動記在 [CHANGELOG.md](CHANGELOG.md)（一行：改哪檔、要不要跟），`template/AGENTS.md` 尾端 `<!-- wf-kernel vX.Y -->` 可 grep 出自己是哪版；kernel-owned（可整檔覆蓋）／project-owned（手動套）見 [IMPORT.md](IMPORT.md)。本 repo 自己的文件在 [docs/](docs/README.md)。
