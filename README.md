...
| [workflows/routines/](template/workflows/routines/README.md) | **常駐精靈型**工作流範例（例行事務 / 定期待辦）：配套 [`.claude/commands/`](template/.claude/commands/)（`/tick`、`/wrapup`）、[`notifier/`](template/notifier/README.md) 通知 daemon、[`state/`](template/state/README.md) 執行期通道、[`launchers/`](template/launchers/README.md) 啟動器。**跨平台**：Windows（`.bat` + PowerShell）/ Linux · macOS（`.sh` + Python）——不需要就整組刪 |
