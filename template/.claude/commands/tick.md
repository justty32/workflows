...
1. **先確認現在的當地時間**（`{{時區，如 Asia/Taipei}}`：幾月幾號、星期幾、幾點幾分）——心跳要靠它判此刻該做哪段、有沒有到期的。依平台取時間：
   - **Windows**（PowerShell；git-bash 的 `TZ=... date` 可能回 GMT，故走 PowerShell）：
     ```powershell
     [System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId((Get-Date), '{{時區 ID，如 Taipei Standard Time}}').ToString('yyyy-MM-dd dddd HH:mm')
     ```
   - **Linux / macOS**（shell，用 IANA 時區名）：
     ```bash
