...
用於初次接觸外部 repo、開源專案、舊系統或大型資料夾時，建立可延續、可搬移、可被後續 planning 使用的分析產物。

**何時用**：初次接觸陌生專案，需要建立可供後續工作延續的架構與能力分析。
**何時不用**：只回答一個窄問題 → [investigation](investigation/README.md)；只是閱讀論文／規格／文章 → research 包（若有導入）；已在目標 repo 內實作功能 → [feature-dev](feature-dev/README.md)。
...
5. 將可行功能或缺口導向 [planning](planning.md)。
...
- 可行功能或缺口 → [planning](planning.md)。
- 窄問題 → [investigation](investigation/README.md)；需使用者驗證 → [WAIT_USER](../WAIT_USER.md)。
