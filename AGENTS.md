...
- 這是**模板 repo**，不是套用了模板的專案：`template/` 是 kernel、`flavors/` 是 flavor 包、`examples/` 是合併成品、`tools/` 是導入與檢查腳本、`docs/` 是本 repo 自己的文件。人的入口 [README.md](README.md)；agent 幫人導入照 [IMPORT.md](IMPORT.md)。
