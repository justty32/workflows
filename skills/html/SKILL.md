...
- Route to [`markdown-html-slides`](../markdown-html-slides/SKILL.md) when the artifact is a slide deck, presentation, or slideshow. Those decks must carry the `deck-theme` / `slides-core` / `slides-runtime` element IDs that `html-slides-shared-assets` relies on; a deck built directly in `html` would silently lack them.
- Continue with `html` for reports, explainers, landing pages, data stories, tools, and mixed artifacts that do not have a clearer owner.
