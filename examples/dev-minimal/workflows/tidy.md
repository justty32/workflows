...
角色分工：**調度者**只盤點、寫交接書、核驗、修跨資料夾／跨 repo 的連結；**實作線**（一個或多個工人）分類、切批、逐批驗收。線**不得**進交接書範圍以外的資料夾或 repo。
...
3. **線跑**：按檔案性質選 A／B 拆法（見 [common/data-files.md](common/data-files.md)：A 法把同質大表抽成資料檔並在 md 端留摘要與查法、對帳列數；B 法把過長散文按語意拆進同名資料夾，原路徑保留當入口）；共通：一律 `git mv`／新檔 `git add`，不留副本，不改內容實質，不重新判定任何狀態。多批可平行，因為各批只動自己範圍內的檔案。
...
<!-- wf-nav -->
- 文件裡的 ```markdown 圍籬內可能刻意放「壞」連結當測試字串；連結改寫器與檢查都要跳過 code fence。
- **目錄改名**會留下正文引註殘留（例如 `` `old-dir/x.md:48` ``），連結檢查抓不到，要另外 grep 舊目錄名。
- 線會把「archive 不維護內部連結」讀成「archive 不能動」而卡住 `broken=0`；交接書要明說：archive 內只准動指向現役檔的「取代者」連結，且 `archive/README.md` 只准**追加**列。
- 表格類活檔要**把規則段留在活檔**、只搬歷史條目，否則線會連規則一起封存。
- 表格裡的「狀態」欄不可信：線收線時多半沒回填，判準改用 `SESSION-LOG.md` 或明確給出的現役清單作為證據，交接書要直接給這個證據，否則線會「不敢猜」留一半。
- 「不搬某個現役目錄」與「清指向 archive 的連結」會在同一批檔上打架；驗收命令要寫成只擋 rename／delete（`git diff --cached --name-status -M <dir>/ | grep -v '^M'`），不要擋 modify。
- 隱藏檔用 `ls -A` 才看得到，`ls` 與大小統計都可能漏掉。
- 線報告裡「需要裁示」的清單是整理的副產物，要照 [WAIT_USER](../WAIT_USER.md) 的格式落地，不能只留在線的回報裡。
